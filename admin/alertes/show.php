...
// ---- TRAITEMENT DE LA CLÔTURE JURIDIQUE (POST) ----
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['resolution_administrative'])) {
    $rapport = nettoyer($_POST['resolution_administrative']);
    
    if (empty($rapport)) {
        $erreur = "Le rapport de résolution final est obligatoire pour clôturer le dossier.";
    } else {
        $conn->begin_transaction();
        try {
            // 1. Passage du litige au statut 'resolu' avec enregistrement du PV final
            $stmt_litige = $conn->prepare("UPDATE litiges SET statut = 'resolu', commentaire = ? WHERE id = ?");
            $stmt_litige->bind_param('si', $rapport, $id);
            $stmt_litige->execute();
            $stmt_litige->close();

            // 2. Libération immédiate de la parcelle (le statut repasse à 'attribue')
            ...
            $conn->commit();
            flashMessage('success', "Le litige a été résolu de manière définitive. La parcelle est libérée au cadastre.");
            header("Location: index.php");
            exit();
...
<div class="container-fluid p-0 flex-grow-1 d-flex flex-column" style="margin-bottom: 110px;">
    ...
    <div class="px-4 py-3 border-bottom bg-white d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-3">
        <div>
            <div class="d-flex align-items-center gap-2 mb-1">
                <small class="text-muted fw-bold text-uppercase tracking-wider" style="font-size: 0.65rem;">Dossier de Litige</small>
            ...
            <h1 class="h4 fw-bold m-0 text-dark">Résolution de Conflit : <?php echo htmlspecialchars($litige['reference']); ?></h1>
        </div>
        <div class="d-flex align-items-center gap-3">
            <?php if ($litige['statut'] === 'ouvert'): ?>
                <span class="badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25 px-3 py-2 rounded-pill d-flex align-items-center gap-1 fw-semibold">
                    <span class="material-symbols-outlined fs-6">warning</span> En Suspens / Bloqué
                </span>
            <?php else: ?>
                <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 px-3 py-2 rounded-pill d-flex align-items-center gap-1 fw-semibold">
            ...
            <div class="d-flex flex-column gap-4">
                ...
                <section>
                    <h3 class="h6 text-dark fw-bold text-uppercase tracking-wider mb-3 pb-2 border-bottom d-flex align-items-center gap-2">
                        <span class="material-symbols-outlined text-primary fs-5">history_edu</span> Historique Judiciaire
                    </h3>
                    <div class="position-relative ps-3 ms-2 border-start border-2 border-light-subtle space-y-4">
                        <div class="position-relative">
                            <div class="position-absolute rounded-circle bg-white border border-primary" style="width: 14px; height: 14px; left: -22px; top: 4px; border-width: 2px !important;"></div>
                            ...
                            <div class="bento-card p-3 bg-light border-0 shadow-none">
                                <h6 class="fw-bold text-dark small mb-1">Ouverture et gel du dossier</h6>
                                <p class="text-muted small m-0">Consigné au registre officiel de Glo-Djigbé par l'agent <?php echo htmlspecialchars($litige['agent_prenom'] . ' ' . $litige['agent_nom']); ?>.</p>
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <section>
                    <h3 class="h6 text-dark fw-bold text-uppercase tracking-wider mb-3 pb-2 border-bottom d-flex align-items-center gap-2">
                        <span class="material-symbols-outlined text-primary fs-5">balance</span> Analyse des Revendications
                    </h3>
                    <div class="row g-3">
                        <div class="col-12">
                            <div class="p-3 rounded-3 border bg-light">
                                <div class="d-flex align-items-center gap-2 text-primary mb-2 fw-bold small">
                                    ...
                                    <span>Titulaire inscrit & Nature du conflit</span>
                                </div>
                                <p class="text-muted small mb-2"><strong>Propriétaire assigné :</strong> <?php echo htmlspecialchars($litige['prop_prenom'] . ' ' . $litige['prop_nom']); ?> (NPI: <?php echo htmlspecialchars($litige['prop_npi']); ?>)</p>
                                <p class="text-dark small mb-0 lh-base">
                                    <strong>Catégorie :</strong> <?php echo ucfirst(str_replace('_', ' ', $litige['type'])); ?><br>
                                    <strong>Faits consignés :</strong> <?php echo htmlspecialchars($litige['description']); ?>
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <section>
                    <h3 class="h6 text-dark fw-bold text-uppercase tracking-wider mb-3 pb-2 border-bottom d-flex align-items-center gap-2">
                        <span class="material-symbols-outlined text-primary fs-5">folder_open</span> Pièces Jointes
                    </h3>
                    <div class="d-flex flex-column gap-2">
                        <div class="p-3 bg-light border rounded-3 d-flex align-items-center justify-content-between">
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div>
                                    <span class="d-block fw-semibold text-dark small text-truncate" style="max-width: 210px;">Rapport_Contradictoire_Geometre.pdf</span>
                                    <small class="text-muted d-block" style="font-size: 0.7rem;">Document technique de bornage</small>
                                </div>
                            </div>
...
<?php if ($litige['statut'] !== 'resolu'): ?>
    <div class="fixed-bottom border-top bg-light p-3 shadow" style="left: var(--sidebar-width, 260px); z-index: 1020;">
        <form method="POST">
            <div class="container-fluid">
                <div class="row align-items-end g-3">
                    <div class="col-12 col-md-7 col-lg-8">
                        <label class="form-label small fw-bold text-dark d-flex align-items-center gap-1 mb-1">
                            <span class="material-symbols-outlined text-primary fs-6">gavel</span> Résolution Administrative
                        </label>
                        <textarea name="resolution_administrative" class="form-control shadow-sm" rows="2" placeholder="Saisir le rapport de résolution final et les conditions de déblocage..." required></textarea>
                    </div>
                    <div class="col-12 col-md-5 col-lg-4">
                        <div class="d-flex gap-2">
                            ...
                            <button type="submit" class="btn text-white fw-bold d-flex align-items-center justify-content-center gap-2 w-50" 
                                    ...
                                    onclick="return confirm('Confirmez-vous le procès-verbal de médiation ? Le litige sera clos et la parcelle redeviendra immédiatement valide pour les ventes.');">
...
<?php else: ?>
    <div class="fixed-bottom border-top bg-white p-3 text-center shadow" style="left: var(--sidebar-width, 260px); z-index: 1020;">
        <span class="text-success fw-bold d-inline-flex align-items-center gap-2 px-4 py-2 bg-success bg-opacity-10 rounded-pill border border-success border-opacity-10">
            <span class="material-symbols-outlined">verified</span> Dossier Arbitré & Résolu — La parcelle a été débloquée au cadastre communal.
        </span>
    </div>
<?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof L !== 'undefined' && lat && lng) {
        ...
        // Rayon rouge symbolisant le périmètre contesté
        L.circle([lat, lng], {
            color: '#EF4444',
            fillColor: '#EF4444',
            fillOpacity: 0.15,
        ...
        var icone = L.divIcon({
            ...
            html: '<span class="material-symbols-outlined text-danger shadow" style="font-size:36px; font-variation-settings: \'FILL\' 1;">location_on</span>',
            iconSize: [36, 36],
            iconAnchor: [18, 36]
        });

        L.marker([lat, lng], { icon: icone }).addTo(carte).bindPopup('<b>⚠️ Zone de Litige : ' + ref + '</b>').openPopup();
    }
});
</script>
