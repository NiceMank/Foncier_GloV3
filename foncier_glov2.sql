...
CREATE TABLE `proprietaires` (
  ...
  `password_connexion` varchar(255) DEFAULT NULL,
  `compte_actif` enum('oui','non') DEFAULT 'non',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `transferts` (
  ...
  `reference` varchar(100) NOT NULL,
  `parcelle_id` int(10) UNSIGNED NOT NULL,
  `ancien_proprietaire_id` int(10) UNSIGNED NOT NULL,
  ...
  `nouveau_email` varchar(150) DEFAULT NULL,
  `adresse` text NOT NULL,
  `document_preuve` varchar(255) NOT NULL,
  `statut` enum('en_attente','en_verification','valide','rejete') NOT NULL DEFAULT 'en_attente',
  `commentaire` text DEFAULT NULL,
  `agent_validateur_id` int(10) UNSIGNED DEFAULT NULL,
...
ALTER TABLE `transferts`
  ...
  ADD KEY `idx_ancien_prop` (`ancien_proprietaire_id`),
  ADD KEY `idx_agent_val` (`agent_validateur_id`);
...
--
ALTER TABLE `transferts`
  ADD CONSTRAINT `fk_transferts_agent` FOREIGN KEY (`agent_validateur_id`) REFERENCES `users` (`id`) ON DELETE SET NULL,
  ADD CONSTRAINT `fk_transferts_parcelle` FOREIGN KEY (`parcelle_id`) REFERENCES `parcelles` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_transferts_proprietaire` FOREIGN KEY (`ancien_proprietaire_id`) REFERENCES `proprietaires` (`id`) ON DELETE CASCADE;
COMMIT;
